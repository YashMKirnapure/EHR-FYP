    ...
    const init = async () => {
      if (window.ethereum) {
        try {
          ...
          setError("⚠️ Failed to load diagnostic details. Please try again.");
        }
      } else {
        setError("⚠️ MetaMask not detected. Please install it.");
      }
      setLoading(false);
    };
    ...
    <div className="bg-gradient-to-b from-[#0a2540] to-[#06172e] min-h-screen">
      ...
      >
        ...
        {!loading && diagnosticDetails && (
          ...
          >
            Welcome,{" "}
            <span className="font-semibold text-cyan-400">{diagnosticDetails[1]}</span>
          </motion.p>
        )}
